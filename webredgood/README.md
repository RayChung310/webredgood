...
1. 建立資料庫：
   ...
   createdb webredgood
   ```
2. 連線設定在 src/main/resources/application.properties application.properties，設定url、username、password即可。
...
- **Swagger UI**：啟動後開啟 **http://localhost:8080/swagger-ui.html** 可查看並測試所有 API。
- **已實作 API 範例**：
  - **查詢網紅粉絲縣市分布**  
    `GET /api/influencers/{id}/followers/distribution`  
    範例：`GET http://localhost:8080/api/influencers/1/followers/distribution`  
    回傳：`influencer`（網紅名稱）、`totalFollowers`（粉絲總數）、`distribution`（各縣市名稱、人數、佔比，依人數降序）。
